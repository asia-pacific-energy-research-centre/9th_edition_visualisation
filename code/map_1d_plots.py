...
def map_9th_data_to_one_dimensional_plots(ECONOMY_ID, EXPECTED_COLS, total_emissions_co2, total_emissions_ch4, total_emissions_co2e, total_emissions_no2):
    ...
    energy_intensity_df, emissions_co2_intensity_df, energy_df, emissions_co2_df = calculate_and_extract_intensity_data(ECONOMY_ID, total_emissions_co2)
    
    all_1d_plotting_dfs = pd.concat([all_1d_plotting_dfs, energy_intensity_df, emissions_co2_intensity_df], axis=0)
    ...
    kaya_identity_df = calculate_and_extract_kaya_identity_data(ECONOMY_ID, energy_df, emissions_co2_df, total_emissions_co2)
...
def calculate_and_extract_intensity_data(ECONOMY_ID, total_emissions_co2):
    ...
    # emissions_co2 = all_model_df_wides_dict['emissions_co2'][1]
    ...
    # emissions_co2_total = emissions_co2[(emissions_co2.sectors == '07_total_primary_energy_supply') & (emissions_co2.subfuels == 'x')].copy()
    # # Group the filtered data by the 'scenarios' column
    # emissions_co2_total = emissions_co2_total.groupby('scenarios').sum().reset_index().copy()
    # unique_emissions_co2_scenarios = emissions_co2_total.scenarios.unique()
    # if len(emissions_co2_total) > len(unique_emissions_co2_scenarios):
    #     breakpoint()
    #     raise Exception('There are more rows in emissions_co2_total than there are unique scenarios. This should not happen.')

    # emissions_co2_total.to_csv('../intermediate_data/emissions_co2_total.csv')
    ...
    # years_cols = [x for x in total_emissions_co2.columns if re.match(r'\d\d\d\d', x)]
    # emissions_co2_total = total_emissions_co2[years_cols + ['scenario']].copy()

    #quickly check for duplicates
    if energy_total.duplicated().any():
        breakpoint()
        raise Exception('There are duplicates in energy_total')
    # if total_emissions_co2.duplicated().any():
    #     breakpoint()
    #     raise Exception('There are duplicates in emissions_co2_total')

    #now melt
    energy_total_melt = energy_total.melt(id_vars=['scenarios'], value_vars=years_cols, var_name='year', value_name='energy_total')
    emissions_co2_total_melt = total_emissions_co2.copy()
    
    # Change the column name scenario to scenarios
    emissions_co2_total_melt.rename(columns={'scenario': 'scenarios'}, inplace=True)
    # emissions_co2_total_melt = total_emissions_co2.melt(id_vars=['scenarios'], value_vars=years_cols, var_name='year', value_name='emissions_co2_total')

    #make year int
    energy_total_melt['year'] = energy_total_melt['year'].astype(int)
    emissions_co2_total_melt['year'] = emissions_co2_total_melt['year'].astype(int)
    ...
    emissions_co2_total_melt = emissions_co2_total_melt[(emissions_co2_total_melt['year'] >= MIN_YEAR) & (emissions_co2_total_melt['year'] <= OUTLOOK_LAST_YEAR)]
    ...
    # real_gdp['value'] = real_gdp['value']/100 #to get 10,000 which seems to be similar magnitude to energy and emissions_co2
    ...
    emissions_co2_intensity = pd.merge(emissions_co2_total_melt, energy_total_melt, how='left', on=['year', 'scenarios'])
    # emissions_co2 intensity is calculated as gross emissions_co2 divided by TPES (CO2 intensity of TPES)
    emissions_co2_intensity['value'] = emissions_co2_intensity['value']/emissions_co2_intensity['energy_total']
    emissions_co2_intensity.drop(columns=['energy_total'], inplace=True)
    
    # Copy and save data for kaya identity calculation
    energy_df = energy_intensity.copy()
    emissions_co2_df = emissions_co2_intensity.copy()
    ...
    emissions_co2_intensity = normalize_to_base_year(emissions_co2_intensity, base_year)
    ...
    emissions_co2_intensity.loc[(emissions_co2_intensity['scenarios'] == 'target') & (emissions_co2_intensity['year'] >= MIN_YEAR) & (emissions_co2_intensity['year'] < OUTLOOK_BASE_YEAR), 'value'] = np.nan
    ...
    emissions_co2_intensity.rename(columns={'scenarios':'scenario'}, inplace=True)
    # #cnacatenate and return
    # energy_and_emissions_co2_intensity = pd.concat([energy_intensity, emissions_co2_intensity], axis=0)
    return energy_intensity, emissions_co2_intensity, energy_df, emissions_co2_df
...
#     emissions_co2_total = emissions_co2[(emissions_co2.sectors == '12_total_final_consumption') & (emissions_co2.fuels == '19_total') & (~emissions_co2.subtotal_layout) & (~emissions_co2.subtotal_results)].copy()
#     unique_emissions_co2_scenarios = emissions_co2_total.scenarios.unique()
#     if len(emissions_co2_total) > len(unique_emissions_co2_scenarios):
#         breakpoint()
#         raise Exception('There are more rows in emissions_co2_total than there are unique scenarios. This should not happen.')
...
#     years_cols = [x for x in energy_total.columns if re.match(r'\d\d\d\d', x)]
#     energy_total =energy_total[years_cols + ['scenarios']].copy()
#     years_cols = [x for x in emissions_co2_total.columns if re.match(r'\d\d\d\d', x)]
#     emissions_co2_total =emissions_co2_total[years_cols + ['scenarios']].copy()

#     #quickly check for duplicates
#     if energy_total.duplicated().any():
#         breakpoint()
#         raise Exception('There are duplicates in energy_total')
#     if emissions_co2_total.duplicated().any():
#         breakpoint()
#         raise Exception('There are duplicates in emissions_co2_total')

#     #now melt
#     energy_total_melt = energy_total.melt(id_vars=['scenarios'], value_vars=years_cols, var_name='year', value_name='energy_total')
#     emissions_co2_total_melt = emissions_co2_total.melt(id_vars=['scenarios'], value_vars=years_cols, var_name='year', value_name='emissions_co2_total')
...
#     emissions_co2_intensity = pd.merge(emissions_co2_total_melt, real_gdp, how='left', on=['year'])
#     emissions_co2_intensity['value'] = emissions_co2_intensity['emissions_co2_total']/emissions_co2_intensity['value']
#     emissions_co2_intensity.drop(columns=[ 'emissions_co2_total'], inplace=True)
...
#     emissions_co2_intensity['unit'] = 'MtCO2/ten_thousand_gdp_2017_usd_ppp'
...
#     # #cnacatenate and return
#     # energy_and_emissions_co2_intensity = pd.concat([energy_intensity, emissions_co2_intensity], axis=0)
#     return energy_intensity, emissions_co2_intensity
...
def calculate_and_extract_kaya_identity_data(ECONOMY_ID, energy_df, emissions_co2_df, total_emissions_co2):
    ...
    total_emissions_co2 = total_emissions_co2[total_emissions_co2['year'].isin([OUTLOOK_BASE_YEAR, OUTLOOK_LAST_YEAR])].copy()
    ...
    for scenario in ['reference', 'target']:
        ...
        emissions_co2_intensity_factor = get_factor(emissions_co2_df[emissions_co2_df['scenarios'] == scenario], OUTLOOK_BASE_YEAR, OUTLOOK_LAST_YEAR)
